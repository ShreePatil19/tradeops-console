...
| PRs merged in v0.3 polish phase | 14 (#66 to #73 plus the agent-routes, eval-scorer, env-sync, PDF-fixture, quota-chip, and Axiom-sink passes) |
| Unit tests | 288 / 288 green across 26 test files |
...
| Add `AXIOM_TOKEN` + `AXIOM_DATASET` env vars to Vercel | Vercel dashboard → Settings → Environment variables, or `pnpm env:sync --file=.env.axiom` | Until set, structured logs only land in Vercel's built-in viewer (no long-term storage, no structured query). Once set, every log event ships to Axiom. | ADR-0004 update |
