...
- **Model + StreamOutput tests** (in flight in [test/model-and-stream-output](https://github.com/ShreePatil19/tradeops-console/tree/test/model-and-stream-output)): 6 new tests for `src/lib/model.ts` (MODEL_ID constant, `requireApiKey` throws on missing/empty env var, error message mentions aistudio URL + .env.local + Vercel), 13 new tests for `StreamOutput` covering empty state (rich + string fallback), loading skeleton, all 4 structured error cards (RateLimitCard, BudgetExhaustedCard, BlockedCard, PayloadTooLargeCard), GenericErrorCard with/without trace_id, and assistant message rendering (text parts render, user messages hidden). Test count: 128.
