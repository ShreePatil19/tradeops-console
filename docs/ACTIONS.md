...
- **Middleware + ToolCallCard tests** (in flight in [test/middleware-and-toolcard](https://github.com/ShreePatil19/tradeops-console/tree/test/middleware-and-toolcard)): 13 integration tests for `src/middleware.ts` (trace propagation, payload-too-large 413, rate limit 429, global budget 429, success fall-through, RateLimit headers, agent slug extraction, x-forwarded-for + x-real-ip handling). 10 component tests for `ToolCallCard` (tool name extraction, status labels for all 4 states, input/output rendering, error state). Mocks `@/lib/rate-limit` + `@/lib/log` via `vi.hoisted` so the imports work with vi.mock hoisting. Test count: 151.
